...
//
//   const sPAPaises = Convert.toSPAPaises(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface SPAPaises {
    name:         Name;
    tld:          string[];
    cca2:         string;
    ccn3:         string;
...
export class Convert {
    public static toSPAPaises(json: string): SPAPaises[] {
        return cast(JSON.parse(json), a(r("SPAPaises")));
    }

    public static sPAPaisesToJson(value: SPAPaises[]): string {
        return JSON.stringify(uncast(value, a(r("SPAPaises"))), null, 2);
    }
}
